    ...
    if df is not None:
        ...
        if chart_type not in ["円グラフ", "ヒストグラム", "箱ひげ図", "バイオリンプロット"]:
            ...
            with st.expander("x軸の目盛り・グリッド詳細"):
                ...
                grid_color_x = c_gc.color_picker("グリッド色", value="#9CA3AF", key="x_grid_color")
        ...
        st.divider()
        st.header("④ y軸の設定")
        grid_color_x = st.session_state.get("x_grid_color", "#9CA3AF")
        grid_alpha_x = st.session_state.get("x_grid_alpha", 0.5)
        y_axis_mapping = {}
        axis_configs = {1: {"name": y_axes[0] if y_axes else "", "unit": "", "min": None, "max": None, "label_size": 18, "tick_size": 14, "major": None, "minor": None, "grid_maj": True, "grid_min": False, "dir": "in", "grid_color": "#9CA3AF", "grid_alpha": 0.5}}
        ...
        if y_axes and chart_type not in ["円グラフ", "ヒストグラム", "箱ひげ図", "バイオリンプロット"]:
            ...
            for idx in sorted(list(active_ids)):
                with st.expander(f"{idx}本目の軸の詳細設定"):
                    ...
                    c_gc, c_ga = st.columns(2)
                    a_grid_color = c_gc.color_picker("グリッド色", value="#9CA3AF", key=f"agrid_color_{idx}")
    ...
    if not y_axes:
        ...
        try:
            if chart_type in ["折れ線グラフ", "散布図", "棒グラフ", "複合グラフ"]:
                ...
                if grid_major_x:
                    ax.grid(which='major', axis='x', linestyle='--', alpha=grid_alpha_x, color=grid_color_x)
                if grid_minor_x:
                    ax.grid(which='minor', axis='x', linestyle=':', alpha=grid_alpha_x * 0.6, color=grid_color_x)
                ...
                for i, target_ax in axes.items():
                    ...
                    if conf.get("grid_maj"):
                        target_ax.grid(which='major', axis='y', linestyle='--', alpha=conf.get("grid_alpha", 0.5), color=conf.get("grid_color", "#9CA3AF"))
                    if conf.get("grid_min"):
                        target_ax.grid(which='minor', axis='y', linestyle=':', alpha=conf.get("grid_alpha", 0.5) * 0.6, color=conf.get("grid_color", "#9CA3AF"))
...
"""
    ...
    st.markdown("""
    ### 使い方
    1. **①データのインポート**: 左側のパネルからCSVファイルを選択します（複数同時に読み込むこともできます）。
    2. **②グラフの種類と軸の選択**: 書きたいグラフの種類と、使いたいデータ（x軸・y軸）を選びます。
    3. **③x軸・④y軸の設定**: 軸の名前や範囲、目盛りの間隔、グリッド線の色などを自分好みに調整します。
    4. **⑤画像の設定**: グラフの大きさやタイトルの有無を決めます。
    5. **画像を保存**: グラフの下にある「画像をダウンロード」ボタンで、レポートに貼れるPNG画像を保存できます。
    
    **💡 ヒント**: 
    - データの傾向を見たいときは「⑥フィッティング設定」で直線近似などを試してみましょう。
    - 複数のファイルを読み込むと、ファイルごとのデータを重ねて比較できます。
    """)
